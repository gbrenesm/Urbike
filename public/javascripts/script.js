...
const paintAllRoutes = async (num) => {
  const res = await fetch(
    "https://datos.cdmx.gob.mx/api/records/1.0/search/?dataset=ciclovias&q=&facet=estado&facet=nombre&facet=vialidad&facet=tipo_ic&facet=tipo_via&facet=instancia"
  );
  const { records } = await res.json();
  var canvas = map.getCanvasContainer();
  const [start] = records[num].fields.geo_shape.coordinates;
  const end = records[num].fields.geo_shape.coordinates[records[num].fields.geo_shape.coordinates.length - 1];
  start.pop();
  end.pop();
  ...
  map.on("load", function () {
    ...
    var coordsObj = records[num].fields.geo_shape.coordinates[
      records[num].fields.geo_shape.coordinates.length - 1
    ];;
  canvas.style.cursor = '';
  var coords = Object.keys(coordsObj).map(function(key) {
    return coordsObj[key];
    ...
    map.on("load", function () {
      map.addSource("route", {
        ...
        data: {
          ...
          geometry: {
            type: "LineString",
            coordinates: records[num].fields.geo_shape.coordinates
          }
        }
      });
      map.addLayer({
